...
class LandingHeader extends Component {
  ...
  componentWillReceiveProps = (nextProps) => {
    ...
    alertMessage = (nextProps.searchErrors && toast.error('Le produit n\' est pas disponible'));
        ...
        <div className='grid-container'>
          ...
          <div className='grid-item search'>
            <form className='search-form' onSubmit={this.handleEnterPress}>
              <span className='all'><span onClick={this.handleSubmitAll}>TOUS</span><div className = "vertical"></div></span>
              <input
                name='searchedItem'
                type='text'
          ...
          <div className='grid-item language' style={{visibility:(!localToken)?'visible':'hidden'}}>
            <a href="https://ricainc.co.com/">ANGLAIS | </a>
            <a href="https://kiny.ricainc.co.com/">KINYA | </a>
            <a href="https://fr.ricainc.co.com/">FRANCAIS</a>
          </div>
            {(!localToken)?
            (<div className='grid-item'><Link to='/signup'>S'inscrire | </Link>
             <Link to='/login'>Connexion</Link></div>):
             (<div className='grid-item'><Link to='/displayclientorders'> Commandes &nbsp;&nbsp;</Link>
            <Link to='' onClick={this.handleLogout}> Se déconnecter</Link></div>)}
          <div className='grid-item navbar'>
            <Link to='/'>Accueil</Link>
            <Link to='#'>Vidéos</Link>
            <span style={{fontWeight:'500'}} onClick={this.handleClick}>Consultante</span>
            <Link to='/displaynews'>Nouvelles</Link>
            {this.state.loggedInUser?(this.state.loggedInUser.type ==='admin'?<Link to='/addproduct'>Admin</Link>:null):null}
            <DropDown 
            visibility={visibility} 
            details={['conception','étude']}
