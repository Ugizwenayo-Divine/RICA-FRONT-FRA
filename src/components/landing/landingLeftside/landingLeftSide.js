...
const { displaySidebar, displayBestProducts } = sidebar;
    ...
    return (
      <div>
        <div
          ...
          style={{ backgroundColor: !loading ? '#f0f0f0e7' : 'white' }}
        >
          <h5 style={{ color: textColor }}>Catégories</h5>
          <ul>
            {!loading && data.length !== 0 ? (
              [...new Set(uniqueData)].map((dt) => (
                <li
                  onClick={() => {
                    this.handleClick(dt);
                  }}
          ...
          <h5 style={{ color: textColor }}>Compagnie</h5>
          <ul>
            {!loading && data.length !== 0 ? (
              [...new Set(uniqueDataBrand)].map((dt) => (
                <li
                  onClick={() => {
                    this.handleClick(dt);
                  }}
          ...
          <h5 style={{ color: textColor }}>Meilleurs produits</h5>
          <ul>
            {!loading && bestProducts.length !== 0 ? (
              bestProducts.map((dt) => (
                <li
                  onClick={() => {
                    this.handleClick(dt.name);
                  }}
                  key={dt.id}
                >
                  <strong>
...
export default connect(mapStateToProps, {
  ...
  displayBestProducts: displayBestProducts,
})(LandingLeft);
