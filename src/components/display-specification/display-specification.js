            ...
            <div style={{width:'100%',height:'100%', display:'inline-flex'}}>
              <div className='container' style={{width:'65%',height:'600px', padding:'10px', backgroundColor:'#f0f0f0e7'}}>
                ...
                <div className='form-group order-button' style={{width:'50%', marginLeft:'0', paddingBottom:'2%'}}>
                  ...
                  <div>{!this.state.token?<small style={{color:'red'}}>Vous commandez uniquement lorsque vous êtes connecté</small>:null}</div>
              </div>
              <div style={{width:'30%', height:'auto',paddingTop:'2%', marginLeft:'5%'}}>
                <h4><strong>Spécifications du produit</strong></h4>
                <hr/>
                <div style={{width:'100%', height:'100%'}}>
                  <p><strong>Nom:</strong> {product.name}</p>
                  <p><strong>Compagnie:</strong> {product.brand}</p>
                  <p><strong>Catégorie:</strong> {product.category}</p>
                  <p><strong>Prix ​​unitaire:</strong> {product.price}</p>
                  <p><strong>Disponible:</strong> {product.quantity} unités</p><br/>
                  <div><strong>Plus de détails</strong> <hr style={{width:'30%', marginLeft:0}}/> {product.description?<Editor editorState={description} readOnly={true} />:null}</div>
                </div>
              </div>
